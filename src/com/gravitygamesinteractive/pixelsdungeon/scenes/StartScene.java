...
import com.gravitygamesinteractive.pixelsdungeon.Chrome;
...
import com.gravitygamesinteractive.pixelsdungeon.ui.Archs;
import com.gravitygamesinteractive.pixelsdungeon.ui.Icons;
import com.gravitygamesinteractive.pixelsdungeon.ui.RedButton;
import com.gravitygamesinteractive.pixelsdungeon.ui.SimpleButton;
import com.gravitygamesinteractive.pixelsdungeon.ui.Window;
import com.gravitygamesinteractive.pixelsdungeon.utils.Utils;
import com.gravitygamesinteractive.pixelsdungeon.windows.WndList;
import com.gravitygamesinteractive.pixelsdungeon.windows.WndOptions;
import com.gravitygamesinteractive.pixelsdungeon.windows.WndTitledMessage;
...
public class StartScene extends PixelScene {
	...
	public void create() {
		...
		GemButton btns[] = {
			new GemButton( HeroClass.KYLE ), 
			new GemButton( HeroClass.IRIS ), 
			new GemButton( HeroClass.CHOMP ), 
			new GemButton( HeroClass.WHOMP ),
			new GemButton( HeroClass.BLAZE ),
			new GemButton( HeroClass.ASH ),
			new GemButton( HeroClass.SHADE ),
			new GemButton( HeroClass.BYTT ),};
		
		float space = width;
		for (GemButton btn : btns) {
			if(space>=width-(btn.width()*3)){
				space -= btn.width();
			}
		}
		...
		for (GemButton btn : btns) {
			add( btn );
			btn.setPos( align( left + p ), align( pos+p2 ) );
			p += btn.width() + space / 3;
			if(p>=(btn.width()+space/3)*4){
				p=0;
				p2+=btn.height();
			}
		}
	...
	private void updateClass( HeroClass cl ) {
		...
		if (cl != HeroClass.WHOMP || huntressUnlocked) {
			...
			switch (curClass) {
			case KYLE:
				badgeToCheck = Badges.Badge.MASTERY_KYLE;
				break;
			case IRIS:
				badgeToCheck = Badges.Badge.MASTERY_IRIS;
				break;
			case CHOMP:
				badgeToCheck = Badges.Badge.MASTERY_CHOMP;
				break;
			case WHOMP:
				badgeToCheck = Badges.Badge.MASTERY_WHOMP;
				break;
			case BLAZE:
				badgeToCheck = Badges.Badge.MASTERY_BLAZE;
				break;
			case ASH:
				badgeToCheck = Badges.Badge.MASTERY_ASH;
				break;
			case SHADE:
				badgeToCheck = Badges.Badge.MASTERY_SHADE;
				break;
			case BYTT:
				badgeToCheck = Badges.Badge.MASTERY_BYTT;
				break;
			}
			btnMastery.active = 
			btnMastery.visible = 
